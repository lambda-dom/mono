...
Conceptually, the `Foldable` typeclass extends this to other types by dropping the uniqueness and the monoid morphism requirements for `foldMap f`. The equation is missing from `Foldable`, so we add it to `MonoFoldable`. The class `MonoPointed` is itself very simple:
